...
add_action( 'wp_ajax_tcc_bulk_delete_general_expense', 'tcc_bulk_delete_general_expense_ajax' );
...
function tcc_load_master_finances_ajax() {
    ...
    foreach($quotes as $q) {
        ...
        $data = !empty($meta_data) ? json_decode($meta_data, true) : json_decode($q->post_content, true);
...
function tcc_save_booking_expense_ajax() {
    ...
    // Safely fetch JSON data from new meta structure with fallback to old post_content
    $meta_data = get_post_meta($quote_id, '_tcc_quote_json_data', true);
    $data = !empty($meta_data) ? json_decode($meta_data, true) : json_decode($post->post_content, true);
...
function tcc_bulk_delete_general_expense_ajax() {
    check_ajax_referer( 'tcc_secure_nonce', 'security' );
    if ( ! is_user_logged_in() ) wp_die();
    ...
    $new_general = array_values( array_filter( $general, function( $ge ) use ( $ids ) {
        return ! in_array( $ge['id'], $ids, true );
    }));
