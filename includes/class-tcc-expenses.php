...
function tcc_enqueue_expense_scripts() {
    ...
    if ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'travel_expense_calculator' ) ) {
        ...
        // Dynamic Cache Busting based on file modification time
        $js_file = plugin_dir_path( dirname(__FILE__) ) . 'assets/js/tcc-expenses.js';
        $cache_buster = file_exists( $js_file ) ? filemtime( $js_file ) : '1.0';
