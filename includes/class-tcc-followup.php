...
function tcc_update_lead_action() {
    check_ajax_referer( 'tcc_secure_nonce', 'security' ); // CSRF Protection Added
...
function tcc_render_followup_dashboard() {
    ...
    foreach($quotes as $q) {
        // Safely fetch JSON data from new meta structure with fallback to old post_content
        $meta_data = get_post_meta($q->ID, '_tcc_quote_json_data', true);
        $content = !empty($meta_data) ? json_decode($meta_data, true) : json_decode($q->post_content, true);
    ...
    jQuery(document).ready(function($) {
            ...
            if (!cEmail || cEmail.trim() === '') {
                ...
                $.post(tcc_ajax_obj.ajax_url, {
                    ...
                    if(res.success) {
                        ...
                        if (typeof tccShowToast === 'function') tccShowToast("Failed to save the new email address.", true);
                        else alert("Failed to save the new email address.");
                    }
                });
            } else {
        ...
        function sendEmailAJAX(quoteId, $btn, originalText) {
            ...
            $.post(tcc_ajax_obj.ajax_url, { 
                ...
                $btn.text(originalText).prop('disabled', false);
                if(res.success) {
                    if (typeof tccShowToast === 'function') tccShowToast(res.data, false);
                    else alert(res.data);
                } else {
                    if (typeof tccShowToast === 'function') tccShowToast("Error: " + res.data, true);
                    else alert("Error: " + res.data);
                }
            }).fail(function() {
                $btn.text(originalText).prop('disabled', false);
                if (typeof tccShowToast === 'function') tccShowToast("Server error occurred while sending email.", true);
                else alert("Server error occurred while sending email.");
            });
        }
