jQuery(document).ready(function($) {
    ...
    // This safely attaches the nonce to EVERY $.post and $.ajax request automatically
    ...
    let geCurrentPage = 1;          // Expense History: current page
    const GE_PER_PAGE = 5;          // Expense History: rows per page
    ...
    function calculateDashboardTotals() {
        ...
        if(filteredGeneral.length === 0) {
            ...
            let totalItems  = filteredGeneral.length;
            let totalPages  = Math.ceil(totalItems / GE_PER_PAGE);
            if (geCurrentPage > totalPages) geCurrentPage = totalPages;
            if (geCurrentPage < 1)          geCurrentPage = 1;

            let pageStart   = (geCurrentPage - 1) * GE_PER_PAGE;
            let pageEnd     = Math.min(pageStart + GE_PER_PAGE, totalItems);
            let pageItems   = filteredGeneral.slice(pageStart, pageEnd);

            // ── Bulk-delete bar (hidden until checkboxes are ticked) ──
            generalHtml += `
            <div id="ge_bulk_bar" style="display:none; align-items:center; gap:8px; margin-bottom:8px; background:#fef2f2; padding:7px 12px; border:1px solid #fecaca; border-radius:6px;">
                <span id="ge_sel_count" style="font-size:12px; color:#dc2626; font-weight:700;">0 selected</span>
                <button type="button" id="ge_bulk_delete_btn" style="background:#dc2626; color:#fff; border:none; padding:5px 12px; border-radius:4px; font-size:12px; font-weight:700; cursor:pointer;">🗑️ Delete Selected</button>
            </div>`;

            // ── Table ──
            generalHtml += `<table style="width:100%; border-collapse:collapse; text-align:left;">
                <tr style="background:#f1f5f9; border-bottom:1px solid #cbd5e1;">
                    <th style="padding:8px; width:32px;"><input type="checkbox" id="ge_select_all" title="Select / Deselect All" style="cursor:pointer; width:15px; height:15px;"></th>
            ...
            pageItems.forEach(e => {
                ...
                let paidByBadge = e.paid_by
                    ? `<span style="font-size:10px; background:#e0f2fe; color:#0284c7; padding:2px 6px; border-radius:3px; font-weight:bold; margin-left:5px;">Paid by: ${paidByName}</span>`
                    : '';

                generalHtml += `<tr style="border-bottom:1px solid #f1f5f9;" class="ge-row">
                    <td style="padding:8px;"><input type="checkbox" class="ge-row-chk" data-id="${e.id}" style="cursor:pointer; width:15px; height:15px;"></td>
                    <td style="padding:8px;">${e.date}</td>
                    <td style="padding:8px;">${e.category}</td>
                    <td style="padding:8px; color:#64748b;">${e.desc || ''} ${paidByBadge}</td>
                    <td style="padding:8px; font-weight:bold; color:#dc2626;">-${expFormatINR(e.amount)}</td>
                    <td style="padding:8px; text-align:right; white-space:nowrap;">
                        <button type="button" class="edit-ge-btn"
                            data-id="${e.id}" data-date="${e.date}" data-cat="${e.category}"
                            data-desc="${safeDesc}" data-amount="${e.amount}" data-paidby="${e.paid_by || ''}"
                            style="background:none; border:none; color:#0369a1; cursor:pointer; text-decoration:underline; margin-right:8px; font-size:12px;">Edit</button>
                        <button type="button" class="del-ge-btn" data-id="${e.id}"
                            style="background:none; border:none; color:#dc2626; cursor:pointer; text-decoration:underline; font-size:12px;">Del</button>
                    </td>
                </tr>`;
            });
            generalHtml += `</table>`;
            ...
            // ── Pagination footer ──
            if (totalPages > 1) {
                let startPg = Math.max(1, geCurrentPage - 2);
                let endPg   = Math.min(totalPages, startPg + 4);
                if (endPg - startPg < 4) startPg = Math.max(1, endPg - 4);

                let pgBtns = '';
                for (let pg = startPg; pg <= endPg; pg++) {
                    let active = (pg === geCurrentPage);
                    pgBtns += `<button type="button" class="ge-page-btn" data-page="${pg}"
                        style="padding:3px 9px; border:1px solid ${active ? '#4f46e5' : '#cbd5e1'}; border-radius:4px;
                               background:${active ? '#4f46e5' : '#fff'}; color:${active ? '#fff' : '#334155'};
                               cursor:pointer; font-size:12px; font-weight:${active ? '700' : '400'};">${pg}</button>`;
                }

                generalHtml += `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:10px; flex-wrap:wrap; gap:6px;">
                    <span style="font-size:11px; color:#64748b;">Showing ${pageStart + 1}–${pageEnd} of ${totalItems} expenses</span>
                    <div style="display:flex; gap:4px; align-items:center;">
                        <button type="button" id="ge_prev_page" data-page="${geCurrentPage - 1}"
                            ${geCurrentPage <= 1 ? 'disabled' : ''}
                            style="padding:3px 9px; border:1px solid #cbd5e1; border-radius:4px;
                                   background:${geCurrentPage <= 1 ? '#f1f5f9' : '#fff'};
                                   color:${geCurrentPage <= 1 ? '#94a3b8' : '#334155'};
                                   cursor:${geCurrentPage <= 1 ? 'default' : 'pointer'}; font-size:12px;">← Prev</button>
                        ${pgBtns}
                        <button type="button" id="ge_next_page" data-page="${geCurrentPage + 1}"
                            ${geCurrentPage >= totalPages ? 'disabled' : ''}
                            style="padding:3px 9px; border:1px solid #cbd5e1; border-radius:4px;
                                   background:${geCurrentPage >= totalPages ? '#f1f5f9' : '#fff'};
                                   color:${geCurrentPage >= totalPages ? '#94a3b8' : '#334155'};
                                   cursor:${geCurrentPage >= totalPages ? 'default' : 'pointer'}; font-size:12px;">Next →</button>
                    </div>
                </div>`;
            } else {
                generalHtml += `<div style="font-size:11px; color:#94a3b8; margin-top:6px; text-align:right;">
                    Showing all ${totalItems} expense${totalItems !== 1 ? 's' : ''}</div>`;
            }
    ...
    $('#frm_general_expense').on('submit', function(e) {
        ...
        $.post(tcc_exp_obj.ajax_url, {
            ...
            if(res.success) { 
                ...
                $('#ge_id').val(''); $('#frm_general_expense')[0].reset(); btn.text('Add'); $('#ge_cancel_edit').hide();
                geCurrentPage = 1; loadMasterData(); 
            } 
            else { btn.text(origText); tccShowToast("Error saving expense.", true); }
        });
    });
    ...
    // ── Select-All checkbox ──
    $(document).on('change', '#ge_select_all', function() {
        $('.ge-row-chk').prop('checked', $(this).is(':checked'));
        geUpdateBulkBar();
    });

    // ── Individual row checkbox ──
    $(document).on('change', '.ge-row-chk', function() {
        let total   = $('.ge-row-chk').length;
        let checked = $('.ge-row-chk:checked').length;
        $('#ge_select_all')
    ...
    function geUpdateBulkBar() {
        let count = $('.ge-row-chk:checked').length;
        if (count > 0) {
            $('#ge_bulk_bar').css('display', 'flex');
            $('#ge_sel_count').text(count + ' selected');
        } else {
            $('#ge_bulk_bar').hide();
        }
    }
    ...
    // ── Bulk delete ──
    $(document).on('click', '#ge_bulk_delete_btn', function() {
        let ids = [];
        $('.ge-row-chk:checked').each(function() { ids.push($(this).data('id')); });
        if (ids.length === 0) return;
        if (!confirm('Permanently delete ' + ids.length + ' selected expense(s)?')) return;

        let $btn = $(this);
        $btn.text('Deleting...').prop('disabled', true);
        $.post(tcc_exp_obj.ajax_url, { action: 'tcc_bulk_delete_general_expense', ids: ids }, function(res) {
            if (res.success) {
                geCurrentPage = 1;
                $(document).trigger('tcc_finances_updated', ['tcc-expenses']);
                loadMasterData();
            } else {
                tccShowToast('Error deleting expenses.', true);
                $btn.text('🗑️ Delete Selected').prop('disabled', false);
            }
        });
    });

    // ── Pagination (Prev / Page-number / Next) ──
    $(document).on('click', '#ge_prev_page, #ge_next_page, .ge-page-btn', function() {
        if ($(this).prop('disabled')) return;
        geCurrentPage = parseInt($(this).data('page'), 10) || 1;
        loadMasterData();
    });
    ...
    $('#frm_auto_expense').on('submit', function(e) {
        ...
        $.post(tcc_exp_obj.ajax_url, {
            ...
            if(res && res.success) { 
                $(document).trigger('tcc_finances_updated', ['tcc-expenses']);
                $('#ae_id').val(''); $('#frm_auto_expense')[0].reset(); $('#ae_cancel_edit').hide(); $('#frm_auto_expense button[type="submit"]').text('Set Recurring'); loadMasterData(); tccShowToast("Recurring expense saved successfully!", false); 
            } 
            else tccShowToast("Error: Data could not be saved. Please check your inputs.", true);
        }).fail(function() { btn.text(origText).prop('disabled', false); tccShowToast("Server Error: Your database could not process the request.", true); });
    });
    ...
    $('#bk_save_manual_btn').on('click', function() {
        ...
        $.post(tcc_exp_obj.ajax_url, data, function(res) {
            ...
            if(res.success) { 
                $(document).trigger('tcc_finances_updated', ['tcc-expenses']);
                loadMasterData(function() { tccShowToast("Booking Finances Updated & Master Sync Successful!", false); }); 
            }
        });
    });
    ...
    $('#frm_partner_setup').on('submit', function(e) {
        ...
        if(otherTotal + newPercent > 100) {
            ...
            tccShowToast(`Cannot save! The maximum available share remaining is ${avail}%. Please adjust your input.`, true);
            return;
        }
