jQuery(document).ready(function($) {
    ...
    function renderAutoExpenses() {
        ...
        if(!masterData.auto_expenses || masterData.auto_expenses.length === 0) {
            ...
            html += `<table style="width:100%; border-collapse:collapse; text-align:left;">
                        <tr style="background:#f1f5f9; border-bottom:1px solid #cbd5e1;">
                            <th style="padding:8px;">Category</th><th style="padding:8px;">Details</th><th style="padding:8px;">Daily Amt</th><th style="padding:8px;">Action</th>
                        </tr>`;
            masterData.auto_expenses.forEach(e => {
                html += `<tr style="border-bottom:1px solid #f1f5f9;">
                            <td style="padding:8px;">${e.category}</td>
                            <td style="padding:8px; color:#64748b;">${e.desc}</td>
                            <td style="padding:8px; font-weight:bold;">${expFormatINR(e.amount)} / day</td>
                            <td style="padding:8px;"><button type="button" class="del-ae-btn" data-id="${e.id}" style="background:none; border:none; color:#dc2626; cursor:pointer; text-decoration:underline;">Stop</button></td>
                         </tr>`;
            });
            html += `</table>`;
        }
    ...
    $(document).on('click', '.del-ae-btn', function() {
        if(!confirm('Stop this auto-daily expense? (Past logs will remain untouched)')) return;
